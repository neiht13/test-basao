...
export interface HamletStats {
    ...
    population?: string;
    partyMembers?: string;
    exemptPartyMembers?: string; // Đảng viên miễn công tác
    partyCellMembers?: string; // Số lượng chi ủy viên
    selfGovGroups?: string; // Số tổ nhân dân tự quản
    mainOccupation?: string; // Nghề chính
}
...
export interface Hamlet {
    ...
    organizations?: string[]; // Danh sách tổ chức CT-XH
    goals?: string; // Mục tiêu/định hướng
    stats: HamletStats;
    leaders: Leader[];
    security?: Leader[];
    images: string[];
