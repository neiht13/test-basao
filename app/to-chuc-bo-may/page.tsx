...
import { UserCircle2, Phone, Mail, Building2, Users, Award, BookOpen } from "lucide-react";
    ...
    return (
        <div className="min-h-screen bg-slate-50 animate-in fade-in duration-700">
            ...
            <div className="container mx-auto px-4 py-16 -mt-10 relative z-20 space-y-16">
                {COMMUNE_ORG_DATA.map((group, groupIdx) => (
                            ...
                            {group.members.map((member, idx) => (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        // Highlight key positions (Bí thư, Chủ tịch)
                                ...
                                >
                                    ...
                                    <div className="flex items-start gap-5 relative z-10">
                                        <div className={cn(
                                            "w-20 h-20 rounded-2xl flex items-center justify-center text-slate-300 shadow-inner border border-slate-100 transition-all duration-300 group-hover:scale-105",
                                            (member.role.includes("Bí thư Đảng uỷ") || member.role.includes("Chủ tịch UBND")) ? "bg-primary text-white shadow-primary/20" : "bg-slate-50 group-hover:bg-white group-hover:text-primary"
                                        )}>
                                        ...
                                        <div className="flex-1 min-w-0 pt-1">
                                            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 text-[10px] font-bold uppercase tracking-wider mb-2 group-hover:bg-primary group-hover:text-white transition-colors">
                                                <Award size={10} />
                                                {member.role.split(',')[0]} {/* Show primary role */}
                                            </div>
